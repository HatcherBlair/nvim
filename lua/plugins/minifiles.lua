...
return {
	{
		...
		opts = {
			...
			content = {
				...
				sort = function(entries)
					local function compare_alphanumerically(e1, e2)
						...
						-- Order numberically based on digits if the text before them is equal
			...
			-- Move stuff to the minifiles trash instead of being gone forever
			options = {
		...
		config = function(_, opts)
			...
			-- Keep track of the explorer being open to prevent format on save
			local minifiles_explorer_group = vim.api.nvim_create_augroup("hatch/minifiles_explorer", { clear = true })
			...
			-- Borrowed from MariaSolOs who borrowed from snacks.nvim
			vim.api.nvim_create_autocmd("User", {
